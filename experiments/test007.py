...
def elite_selection(population, elite_size):
    return sorted(population, key=lambda layout: calculate_typing_cost(word_to_optimize, layout)[0])[:elite_size]

def optimize_keyboard_layout(word_to_optimize):
    population_num = 100
    generation_num = 1000
    elite_size = 20  # Elite size
    ...
    for generation in range(generation_num):
        while True:
            elite_population = elite_selection(population, elite_size)
            parent1, parent2 = random.sample(elite_population, 2)
            child_layout = uniform_crossover(parent1, parent2)
